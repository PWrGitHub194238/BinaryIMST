...
void AIMSTSolver::getIMST(ThreadId threadId, GraphIF* const graph,
	...
	MSTSolverIF* mstSolver = SolverFactory::getMSTSolver(mstSolverType, graph);
	IMSTSolverIF* imstSolver = new IMSTSolverImpl { mstSolver, graph,
			baseSolution };
	outputSolution = imstSolver->getMST(k, scenario);
	delete imstSolver;
	delete mstSolver;
	...
	if (numberOfScenarios > 0) {
		...
		for (i = 0; i < numberOfScenarios; i += 1) {
			...
			/*scenarioThreads[i] = std::thread(&AIMSTSolver::getIMST, this, i,
			 threadGraphSet[i], baseSolution, *scenarioIt,
